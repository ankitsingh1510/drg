...
import { ActivityIndicator, FlatList, Platform, StatusBar, TextInput, TouchableOpacity, View } from 'react-native';
...
const StatusBadge = React.memo(({ status, isDark }: { status: OrderDisplayStatus; isDark: boolean }) => {
  ...
  return (
    ...
    >
      <AppText className="text-sm" style={{ color: isDark ? style.darkText : style.text }}>
        {status}
      </AppText>
    </View>
  );
  ...
  return (
    ...
    >
      <View className="mb-1 flex-row items-center justify-between">
        <AppText weight="semibold" className="flex-1 text-2xl text-gray-900 dark:text-white">
          {patient.patientName}
        </AppText>
        <ChevronRight size={24} color={chevronColor} />
      </View>
      <AppText className="mb-2 text-base text-gray-500 dark:text-[#8BA5C0]">
      ...
      <View className="flex-row flex-wrap gap-4">
        {patient.orders.map((order, idx) => (
          <View key={idx} className="min-w-[30%] items-center justify-center">
            <AppText weight="medium" className="text-lg text-gray-700 dark:text-gray-300">
              {order.testName}
            </AppText>
  ...
  const ListEmptyComponent = useMemo(
    () => (
      <View className="mt-16 items-center justify-center">
        <AppText className="text-base text-gray-500 dark:text-[#8BA5C0]">No orders found.</AppText>
      </View>
    ),
    []
  );
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#FDF5E6] dark:bg-[#111827]">
      ...
      <View className="mb-3 px-4">
        <View className="h-14 flex-row items-center justify-center rounded-2xl border border-gray-200 bg-white px-4 dark:border-[#374151] dark:bg-[#1f2937]">
          <Search size={20} color="#9CA3AF" style={{ marginRight: 8 }} />
          <TextInput
            className="h-14 flex-1 font-outfit text-lg text-gray-800 dark:text-white"
        ...
        {totalCount > 0 && (
          <AppText className="ml-2 mt-2 text-sm uppercase tracking-widest text-gray-500 dark:text-[#8BA5C0]">
      ...
      {showSpinner ? (
        <View className="mt-16 items-center justify-center">
          ...
          <AppText className="mt-3 text-base text-gray-500 dark:text-[#8BA5C0]">Loading orders...</AppText>
        </View>
      ) : showError ? (
        <View className="mt-16 items-center justify-center px-4">
          <AppText className="text-center text-base text-red-500">{error}</AppText>
          <TouchableOpacity
            className="mt-4 rounded-xl bg-blue-600 px-6 py-2"
            onPress={() => fetchOrders(search.trim(), 1)}
          >
            <AppText className="text-base text-white">Retry</AppText>
          </TouchableOpacity>
        </View>
      ) : (
        <FlatList
